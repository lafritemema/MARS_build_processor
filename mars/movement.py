...
class Configuration:
    ...
    """
    def __init__(self, wrist:WristConfig, forearm: ForeArmConfig, arm:ArmConfig, j4:int=0, j5:int=0, j6:int=0)  -> 'Configuration':
        """Configuration object initializer

        Args:
            wrist (WristConfig): wrist configuration enumeration (FLIP|NOFLIP)
            forearm (ForeArmConfig): forearm configuration enumeration (UP|DOWN)
            arm (ArmConfig): arm configuration enumeration (TOWARD|BACKWARD)
...
class Position:
    ...
    def __init__(self, pvector:np.array, ptype:PositionType, e1:int, config:Configuration=None) -> 'Position':
        """ 
...
class PositionCrt(Position):
    ...
    def __init__(self, pvector:np.array, e1:int, config:Configuration) -> 'PositionCrt':
    ...
    def to_dict(self) -> Dict:
        ...
        tl = [(vector_keys[i], float(val)) for i, val in enumerate(self._vector.tolist())]
        p_dict =  dict((key.replace('_Position', ''),val) for (key, val) in self.__dict__.copy().items())
...
class Point:
    ...
    """
    def __init__(self, cnt:int, speed:int, path:Path, position:Position)-> 'Point':
