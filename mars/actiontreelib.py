...
class ActionNode(Node):
    ...
    def __init__(self, action:Action=None) -> 'ActionNode':
        ...
        Args:
            action (Action, optional): The action represented by the node in the tree. Defaults to None.
        """
        ...
        super(ActionNode, self).__init__(action.description if action else 'root',
                                         action.id if action else 0, data=action)
...
class ActionTree(Tree):
    ...
    def __add_action_node(self, action_node:ActionNode, parent:ActionNode):
        """ add an ActionNode in the tree if not allready exist (id already present)
